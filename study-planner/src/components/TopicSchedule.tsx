...
export default function TopicSchedule({ plan, progress }: TopicScheduleProps) {
    ...
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 p-4 md:p-6 h-full overflow-hidden flex flex-col transition-colors">
            <h2 className="text-lg font-bold text-zinc-800 dark:text-zinc-100 mb-4 md:mb-6 flex items-center gap-2">
                        ...
                        return (
                            <div key={idx} className={cn(
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className={cn("text-sm font-bold mb-1", isCurrent ? "text-orange-700 dark:text-orange-400" : "text-zinc-900 dark:text-zinc-100")}>
                                            {format(date, 'MMM d, yyyy')}
                                        </div>
                                        <div className="text-xs text-zinc-500">{format(date, 'EEEE')}</div>
                                    </div>
                                    <span className={cn(
                                ...
                                <div className="flex items-center gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-700/50">
                                    <span className="text-xs text-zinc-500">Status:</span>
                                    {isDone ? (
                                        <div className="flex items-center gap-1 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded text-[10px] font-bold">
                                            <CheckCircle2 className="w-3 h-3" /> Done
                                        </div>
                                    ) : isLate ? (
                                        <div className="flex items-center gap-1 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded text-[10px] font-bold">
                                            <AlertCircle className="w-3 h-3" /> Late
                                        </div>
                                    ) : isCurrent ? (
                                        <div className="flex items-center gap-1 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 px-2 py-0.5 rounded text-[10px] font-bold">
