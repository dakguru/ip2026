...
export default function SocialClient({ initialPosts }: SocialClientProps) {
    ...
    useEffect(() => {
        if (user && feedData.length > 0) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const myIds = feedData.filter((p: any) => p.author === user.name).map((p: any) => p.id);
    ...
    const handleAskQuestion = () => {
        checkAuthAndExecute(async () => {
            ...
            try {
                ...
                if (res.ok) {
                    await fetchPosts(); // Re-fetch to get the latest state including the new post
                    setQuestionInput("");
                    setDetailsInput("");
    ...
    const getFeedData = () => {
        ...
        if (activeTab === "My Questions") {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return feedData.filter((post: any) => myQuestionIds.includes(post.id));
        }
        if (activeTab === "Saved Answers") {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return feedData.filter((post: any) => savedPostIds.includes(post.id));
        }
