    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-indigo-500 selection:text-white overflow-hidden relative">
            ...
            <div className="max-w-7xl mx-auto px-6 py-12 relative z-10">
                ...
                >
                    <Link href="/" className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition-colors group mb-12">
                ...
                <div className="text-center max-w-3xl mx-auto mb-20">
                    ...
                    <motion.h1
                        ...
                        className="text-5xl md:text-7xl font-black tracking-tight mb-6 bg-clip-text text-transparent bg-gradient-to-r from-white via-indigo-100 to-slate-400"
                    >
                        Master the Rules.
                    </motion.h1>
...
function GuideCard({ href, title, badge, description, icon, color, delay }: any) {
    ...
    return (
        ...
        >
            <Link href={href} className={`group relative block h-full bg-slate-900/50 backdrop-blur-sm rounded-3xl p-8 border border-slate-800 hover:-translate-y-2 transition-all duration-300 hover:shadow-2xl ${colorVariants[color]}`}>
                {/* Gradient Overlay on Hover */}
                <div className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 bg-gradient-to-br ${colorVariants[color]} transition-opacity duration-500 pointer-events-none`} />

                <div className="relative z-10 flex flex-col h-full">
                    <div className="flex justify-between items-start mb-6">
                        <div className={`p-4 rounded-2xl transition-colors duration-300 ${iconColors[color]}`}>
                            {icon}
                        </div>
                        {badge && (
                            <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-semibold uppercase tracking-wider text-slate-300 group-hover:bg-white/10 transition-colors">
                                {badge}
                            </span>
                        )}
                    </div>

                    <h2 className="text-3xl font-bold text-white mb-3 group-hover:text-white transition-colors">{title}</h2>
                    ...
                    <div className="flex items-center text-sm font-bold uppercase tracking-widest text-slate-500 group-hover:text-white transition-colors">
                        Explore Guide <ChevronRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                    </div>
                </div>
            </Link>
        </motion.div>
