import { WEEKLY_MOCK_01_QUESTIONS } from "@/data/weekly_mock_data_01";
import { WEEKLY_MOCK_02_QUESTIONS } from "@/data/weekly_mock_data_02";
import { WEEKLY_MOCK_03_QUESTIONS } from "@/data/weekly_mock_data_03";
import { WEEKLY_MOCK_04_QUESTIONS } from "@/data/weekly_mock_data_04";
import { WEEKLY_MOCK_05_QUESTIONS } from "@/data/weekly_mock_data_05";
import { WEEKLY_MOCK_06_QUESTIONS } from "@/data/weekly_mock_data_06";
import { WEEKLY_MOCK_07_QUESTIONS } from "@/data/weekly_mock_data_07";
import { WEEKLY_MOCK_08_QUESTIONS } from "@/data/weekly_mock_data_08";
import { WEEKLY_MOCK_09_QUESTIONS } from "@/data/weekly_mock_data_09";
import { WEEKLY_MOCK_10_QUESTIONS } from "@/data/weekly_mock_data_10";
import { WEEKLY_MOCK_11_QUESTIONS } from "@/data/weekly_mock_data_11";
import { WEEKLY_MOCK_12_QUESTIONS } from "@/data/weekly_mock_data_12";
import { WEEKLY_MOCK_13_QUESTIONS } from "@/data/weekly_mock_data_13";
import { WEEKLY_MOCK_14_QUESTIONS } from "@/data/weekly_mock_data_14";
import { WEEKLY_MOCK_15_QUESTIONS } from "@/data/weekly_mock_data_15";
import { WEEKLY_MOCK_16_QUESTIONS } from "@/data/weekly_mock_data_16";
import { WEEKLY_MOCK_17_QUESTIONS } from "@/data/weekly_mock_data_17";
import { WEEKLY_MOCK_S2_01_QUESTIONS } from "@/data/weekly_mock_data_s2_01";
import { WEEKLY_MOCK_S2_02_QUESTIONS } from "@/data/weekly_mock_data_s2_02";
import { WEEKLY_MOCK_S2_03_QUESTIONS } from "@/data/weekly_mock_data_s2_03";
import { WEEKLY_MOCK_S2_04_QUESTIONS } from "@/data/weekly_mock_data_s2_04";
import { WEEKLY_MOCK_S2_05_QUESTIONS } from "@/data/weekly_mock_data_s2_05";
import { WEEKLY_MOCK_S2_06_QUESTIONS } from "@/data/weekly_mock_data_s2_06";
import { WEEKLY_MOCK_S2_07_QUESTIONS } from "@/data/weekly_mock_data_s2_07";
import { WEEKLY_MOCK_S2_08_QUESTIONS } from "@/data/weekly_mock_data_s2_08";
import { WEEKLY_MOCK_S2_09_QUESTIONS } from "@/data/weekly_mock_data_s2_09";
import { WEEKLY_MOCK_S2_10_QUESTIONS } from "@/data/weekly_mock_data_s2_10";
import { WEEKLY_MOCK_S2_11_QUESTIONS } from "@/data/weekly_mock_data_s2_11";
import { WEEKLY_MOCK_S2_12_QUESTIONS } from "@/data/weekly_mock_data_s2_12";

import { WEEKLY_MOCK_S2_13_QUESTIONS } from "@/data/weekly_mock_data_s2_13";
import { WEEKLY_MOCK_S2_14_QUESTIONS } from "@/data/weekly_mock_data_s2_14";
import { WEEKLY_MOCK_S2_15_QUESTIONS } from "@/data/weekly_mock_data_s2_15";

import { LIVE_MOCK_QUESTIONS } from "@/data/live_mock_data";
import { PSGB_MOCK_01_QUESTIONS } from "@/data/psgb_mock_data_01";
import { PSGB_MOCK_02_QUESTIONS } from "@/data/psgb_mock_data_02";
import { PSGB_MOCK_03_QUESTIONS } from "@/data/psgb_mock_data_03";
import { PSGB_MOCK_04_QUESTIONS } from "@/data/psgb_mock_data_04";
import { PSGB_MOCK_05_QUESTIONS } from "@/data/psgb_mock_data_05";
import { PSGB_MOCK_06_QUESTIONS } from "@/data/psgb_mock_data_06";
import { PSGB_MOCK_07_QUESTIONS } from "@/data/psgb_mock_data_07";
import { PSGB_MOCK_08_QUESTIONS } from "@/data/psgb_mock_data_08";
import { PSGB_MOCK_09_QUESTIONS } from "@/data/psgb_mock_data_09";
import { PSGB_MOCK_10_QUESTIONS } from "@/data/psgb_mock_data_10";
import { PSGB_MOCK_11_QUESTIONS } from "@/data/psgb_mock_data_11";
import { PSGB_MOCK_12_QUESTIONS } from "@/data/psgb_mock_data_12";
import { PSGB_MOCK_13_QUESTIONS } from "@/data/psgb_mock_data_13";
import { PSGB_MOCK_14_QUESTIONS } from "@/data/psgb_mock_data_14";
import { PSGB_MOCK_15_QUESTIONS } from "@/data/psgb_mock_data_15";
import { PSGB_MOCK_16_QUESTIONS } from "@/data/psgb_mock_data_16";

export const TEST_QUESTIONS_MAP: Record<string, any[]> = {
    "mock-2026-01-17": WEEKLY_MOCK_01_QUESTIONS,
    "mock-2026-01-24": WEEKLY_MOCK_02_QUESTIONS,
    "mock-2026-01-31": WEEKLY_MOCK_03_QUESTIONS,
    "mock-2026-02-07": WEEKLY_MOCK_04_QUESTIONS,
    "mock-2026-02-14": WEEKLY_MOCK_05_QUESTIONS,
    "mock-2026-02-21": WEEKLY_MOCK_06_QUESTIONS,
    "mock-2026-02-28": WEEKLY_MOCK_07_QUESTIONS,
    "mock-2026-03-07": WEEKLY_MOCK_08_QUESTIONS,
    "mock-2026-03-14": WEEKLY_MOCK_09_QUESTIONS,
    "mock-2026-03-21": WEEKLY_MOCK_10_QUESTIONS,
    "mock-2026-03-28": WEEKLY_MOCK_11_QUESTIONS,
    "mock-2026-04-04": WEEKLY_MOCK_12_QUESTIONS,
    "mock-2026-04-11": WEEKLY_MOCK_13_QUESTIONS,
    "mock-2026-04-18": WEEKLY_MOCK_14_QUESTIONS,
    "mock-2026-04-25": WEEKLY_MOCK_15_QUESTIONS,
    "mock-2026-05-02": WEEKLY_MOCK_16_QUESTIONS,
    "mock-2026-05-09": WEEKLY_MOCK_17_QUESTIONS,
    "mock-s2-2026-05-30": WEEKLY_MOCK_S2_01_QUESTIONS,
    "mock-s2-2026-06-06": WEEKLY_MOCK_S2_02_QUESTIONS,
    "mock-s2-2026-06-13": WEEKLY_MOCK_S2_03_QUESTIONS,
    "mock-s2-2026-06-20": WEEKLY_MOCK_S2_04_QUESTIONS,
    "mock-s2-2026-06-27": WEEKLY_MOCK_S2_05_QUESTIONS,
    "mock-s2-2026-07-04": WEEKLY_MOCK_S2_06_QUESTIONS,
    "mock-s2-2026-07-11": WEEKLY_MOCK_S2_07_QUESTIONS,
    "mock-s2-2026-07-18": WEEKLY_MOCK_S2_08_QUESTIONS,
    "mock-s2-2026-07-25": WEEKLY_MOCK_S2_09_QUESTIONS,
    "mock-s2-2026-08-01": WEEKLY_MOCK_S2_10_QUESTIONS,
    "mock-s2-2026-08-08": WEEKLY_MOCK_S2_11_QUESTIONS,
    "mock-s2-2026-08-15": WEEKLY_MOCK_S2_12_QUESTIONS,
    "mock-s2-2026-08-22": WEEKLY_MOCK_S2_13_QUESTIONS,
    "mock-s2-2026-08-29": WEEKLY_MOCK_S2_14_QUESTIONS,
    "mock-s2-2026-09-05": WEEKLY_MOCK_S2_15_QUESTIONS,
    "live-sample": LIVE_MOCK_QUESTIONS,
    "psgb-mock-2026-04-05": PSGB_MOCK_01_QUESTIONS,
    "psgb-mock-2026-04-12": PSGB_MOCK_02_QUESTIONS,
    "psgb-mock-2026-04-19": PSGB_MOCK_03_QUESTIONS,
    "psgb-mock-2026-04-26": PSGB_MOCK_04_QUESTIONS,
    "psgb-mock-2026-05-03": PSGB_MOCK_05_QUESTIONS,
    "psgb-mock-2026-05-10": PSGB_MOCK_06_QUESTIONS,
    "psgb-mock-2026-05-17": PSGB_MOCK_07_QUESTIONS,
    "psgb-mock-2026-05-24": PSGB_MOCK_08_QUESTIONS,
    "psgb-mock-2026-05-31": PSGB_MOCK_09_QUESTIONS,
    "psgb-mock-2026-06-07": PSGB_MOCK_10_QUESTIONS,
    "psgb-mock-2026-06-14": PSGB_MOCK_11_QUESTIONS,
    "psgb-mock-2026-06-21": PSGB_MOCK_12_QUESTIONS,
    "psgb-mock-2026-06-28": PSGB_MOCK_13_QUESTIONS,
    "psgb-mock-2026-07-05": PSGB_MOCK_14_QUESTIONS,
    "psgb-mock-2026-07-12": PSGB_MOCK_15_QUESTIONS,
    "psgb-mock-2026-07-19": PSGB_MOCK_16_QUESTIONS
};
