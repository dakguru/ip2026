...
const mcqFile = 'D:\\IP 2026\\MCQs\\GFR_Chapter6_MCQs.txt';
...
// Next ID from quizzes.ts
let quizzesContent = fs.readFileSync(quizzesFile, 'utf8');
const m = quizzesContent.match(/(\d+):\s*\[/g);
const maxId = m ? Math.max(...m.map(s => parseInt(s.match(/\d+/)[0]))) : 0;
...
for (let i = 0; i < setChunks.length; i++) {
    const id = currentId++;
    setIds.push(id);
    const varName = `gfr_chap6_set${i + 1}`;
    imports.push(`import { ${varName} } from './flashcards/gfrChapter6';`);
...
fs.writeFileSync(path.join(flashcardsDir, 'gfrChapter6.ts'), outputJS);
...
const oldLine = `createTopic('psgb-30', "Chapter 6 of General Financial Rules, 2017", 'Paper I', [])`;
const newLine = `createTopic('psgb-30', "Chapter 6 of General Financial Rules, 2017", 'Paper I', [${setIds.join(', ')}])`;
psgbContent = psgbContent.replace(oldLine, newLine);
// Let's also handle case where it might have some ids or single quotes vs double quotes.
// The exact string in the file: createTopic('psgb-30', "Chapter 6 of General Financial Rules, 2017", 'Paper I', [])
// We checked this earlier.
fs.writeFileSync(psgbFile, psgbContent);

console.log("Inserted " + allMcqs.length + " questions into " + setChunks.length + " sets. Assigned IDs: " + setIds.join(', '));
