const fs = require('fs');
const text = fs.readFileSync('d:/IP 2026/MCQs FHB Vol II.txt', 'utf8');
const qs = text.split(/\n\s*\n/).filter(x => x.trim() !== '');
const out = [];
...
for (const q of qs) {
    ...
    const aLine = lines.find(l => l.startsWith('Correct Answer:'));
    if (!aLine) continue;
    ...
    const eText = eLine ? eLine.replace(/^Explanation:\s*/, '').trim() : '';
...
let finalStr = '';
let currentSetId = 146; // From Max Set ID + 1
let generatedSetIds = [];
...
for (const chunk of chunked) {
    ...
    finalStr += `  ],\n`;
    generatedSetIds.push(currentSetId);
    currentSetId++;
}

fs.writeFileSync('d:/IP 2026/tmp_mcqs_formatted_fhb2.txt', finalStr);
console.log('Formatted sets:', generatedSetIds);
